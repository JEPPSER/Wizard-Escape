...
public class Controls implements KeyListener, MouseListener{
	...
	public Controls(PlatformMain platform){
	...
	public void mousePressed(MouseEvent e) {
		if(pm.enter == false && pm.enterCount == 0 &&
			e.getX() > pm.startGame.x && e.getX() < pm.startGame.x + pm.startGame.width &&
			e.getY() > pm.startGame.y + 37 && e.getY() < pm.startGame.y + pm.startGame.height + 37){
			
			pm.enter = true;
		}
		else if(pm.enter == false && pm.enterCount == 0 &&
			e.getX() > pm.howToPlay.x && e.getX() < pm.howToPlay.x + pm.howToPlay.width &&
			e.getY() > pm.howToPlay.y + 37 && e.getY() < pm.howToPlay.y + pm.howToPlay.height + 37){
